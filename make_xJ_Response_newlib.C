...
void make_xJ_Response_newlib(string infile1 = "All-Herwig-Jet10-R04-Run21.root", const std::string configfile = "binning_original.config"){
...
//Define Histograms
   ...
   tree->SetBranchAddress("truth_jet_pt_4",&tjet_pt); tree->SetBranchAddress("truth_jet_eta_4",&tjet_eta); tree->SetBranchAddress("truth_jet_phi_4",&tjet_phi);tree->SetBranchAddress("truth_vertex_z",&t_z_vtx);

   std::vector<double> *rjet_pt ={0}; std::vector<double> *rjet_phi ={0}; std::vector<double> *rjet_eta ={0}; float r_z_vtx = 0;
      tree->SetBranchAddress("jet_pt_4",&rjet_pt); tree->SetBranchAddress("jet_eta_4",&rjet_eta); tree->SetBranchAddress("jet_phi_4",&rjet_phi);tree->SetBranchAddress("mbd_vertex_z",&r_z_vtx);
      ...
      //define cuts
      Double_t dphicut = rb.get_dphicut(); float RValue = 0.4; float etacut = 1.1 - RValue; 
 ...
 // identify file to determine setting
  ...
  if (infile1.find("Jet30") != std::string::npos){
    setting = 30; minpt = 30;  maxpt = 200;weight = 2.505*pow(10,-9);
     if(infile1.find("Herwig") != std::string::npos){
       weight = weight*2.27; cout << "Herwig detected" << endl;
    }
    cout << "30 GEV  JET INFILE" << endl;
   }
  else if (infile1.find("Jet10") != std::string::npos){
    setting = 10; minpt = 10;  maxpt = 30; weight = 3.646*pow(10,-6);
    if(infile1.find("Herwig") != std::string::npos){
      weight = weight*1.15;
		   ...
		   // evaluate dphi
		       ...
		       if ( rdeltaphi > TMath::Pi())
			{
			  rdeltaphi = 2*TMath::Pi()- rdeltaphi;
		       ...
		       bool truth_good = (tleadpt >= truth_leading_cut && tsubleadpt >= truth_subleading_cut && tdeltaphi > dphicut && fabs(rzvtx) && fabs(tleadeta) < etacut && fabs(tsubleadeta) < etacut && tleadpt >= minpt && tleadpt <= maxpt);
 ...
 xj_functions::clean_pt1pt2(h_pt1pt2_reco_half_test,nbins,weight);
    ...
    //define file for writing
     ...
     std::string nbinsstring = "_Bins";
      ...
      outfilename.Prepend("Xj_2D_Response_Newlib"+ nbinsstring + "_");
