...
void make_xJ_Response_smear(const std::string configfile = "binning_posJES.config", float RValue = 0.4, string infile1 = "pythia8-Jet10-Run21-multiR.root"){
   ...
   //int Nentries = 100000;
	...
	if (ntjets < 2){
	  ...
	  if (JES_sys != 0)
	    ...
	    {
	      rleadpt = rleadpt +  smear1*tleadpt;
	      rsubleadpt = rsubleadpt + smear2*tsubleadpt; 
	    }
 ...
 xj_functions::clean_pt1pt2(h_pt1pt2_reco_half_test,nbins,weight);
    ...
    //define file for writing
     ...
     TString responsepath = "_Smear";
      ...
      outfilename.Prepend("Xj_2D_Response" + nbinsstring + "_R" + Rvaluestring + responsepath);
