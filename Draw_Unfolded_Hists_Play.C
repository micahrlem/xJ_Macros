...
std::vector<std::string> filenames = {
    // Pythia
    "UnfoldedxJHists_Play_pythia8_R2.root",
    "UnfoldedxJHists_Play_pythia8_R4.root",
    "UnfoldedxJHists_Play_pythia8_negJER_R2.root",
    "UnfoldedxJHists_Play_pythia8_negJER_R4.root",
    "UnfoldedxJHists_Play_pythia8_posJER_R2.root",
    "UnfoldedxJHists_Play_pythia8_posJER_R4.root",
    "UnfoldedxJHists_Play_pythia8_negJES_R2.root",
    "UnfoldedxJHists_Play_pythia8_negJES_R4.root",
    "UnfoldedxJHists_Play_pythia8_posJES_R2.root",
    "UnfoldedxJHists_Play_pythia8_posJES_R4.root",

    // Herwig Reweight (with test prefix)
    "UnfoldedxJHists_Play_Herwig_R2.root",
    "UnfoldedxJHists_Play_Herwig_R4.root",
    "UnfoldedxJHists_Play_Herwig_negJER_R2.root",
    "UnfoldedxJHists_Play_Herwig_negJER_R4.root",
    "UnfoldedxJHists_Play_Herwig_posJER_R2.root",
    "UnfoldedxJHists_Play_Herwig_posJER_R4.root",
    "UnfoldedxJHists_Play_Herwig_negJES_R2.root",
    "UnfoldedxJHists_Play_Herwig_negJES_R4.root",
    "UnfoldedxJHists_Play_Herwig_posJES_R2.root",
    "UnfoldedxJHists_Play_Herwig_posJES_R4.root"
};
...
std::vector<std::string> gens = {"Play_pythia8", "Play_Herwig"};
...
for (const auto& pt_range : pt_ranges_compare) {
    ...
    std::string tag_r4_pythia = pt_range + "_Play_pythia8_R4";
    std::string tag_r4_herwig = pt_range + "_Play_Herwig_R4";
    std::string tag_r2_pythia = pt_range + "_Play_pythia8_R2";
    std::string tag_r2_herwig = pt_range + "_Play_Herwig_R2";
    ...
    std::string outpath = "xj_plot_play_newJER_pdfs/" + canvas_key + ".pdf";
    ...
    for (const auto& pt_range : pt_ranges_closure) {
        for (const auto& gen : gens) {
            for (const auto& R : Rvals) {
                ...
                std::string simtype     = (gen == "Play_pythia8") ? "PYTHIA 8" : "HERWIG 7";
...
for (const auto& pt_range : pt_ranges_variation) {
    for (const auto& gen : gens) {
        for (const auto& R : Rvals) {
            ...
            std::string outpath_jer = "xj_plot_play_newJER_pdfs/" + canvas_jer_key + ".pdf";
 ...
 for (const auto& pt_range : pt_ranges_variation) {
  for (const auto& gen : gens) {
    for (const auto& R : Rvals) {
      ...
      // Must have truth, central unfold, and +JER
      bool fatal_missing = false;
      auto need = [&](const std::string& key){
        if (!named_histograms.count(key) || !named_histograms[key]) {
          std::cerr << "Missing histogram: " << key << std::endl;
          return true;
        }
        return false;
      };
      fatal_missing |= need(h_truth);
      fatal_missing |= need(h_unfold);
      fatal_missing |= need(h_jer_pos);
      if (fatal_missing) continue;

      // Handle negJER:
      // - If present, use it.
      // - If absent and R=="R2", synthesize by reflection: h_neg = 2*h_unfold - h_pos
      // - If absent and R!="R2", skip (still truly missing).
      bool have_neg = (named_histograms.count(h_jer_neg) && named_histograms[h_jer_neg]);
      if (!have_neg) {
        if (R == "R2") {
          TH1D* hC = dynamic_cast<TH1D*>(named_histograms[h_unfold]);
          TH1D* hP = dynamic_cast<TH1D*>(named_histograms[h_jer_pos]);
          if (!hC || !hP) { std::cerr << "Central or +JER missing after check? " << base_tag << std::endl; continue; }

          // Build reflected -JER
          std::string synth_name = h_jer_neg + "_synth";
          TH1D* hN = (TH1D*)hC->Clone(synth_name.c_str());
          hN->SetDirectory(nullptr);
          for (int i = 1; i <= hN->GetNbinsX(); ++i) {
            double c = hC->GetBinContent(i);
            double p = hP->GetBinContent(i);
            double epos = hP->GetBinError(i);
            double n = 2.0*c - p;          // reflection
            if (n < 0.0) n = 0.0;          // avoid negative bins
            hN->SetBinContent(i, n);
            hN->SetBinError(i, epos);      // symmetric treatment
          }
          // (optional) mirror under/overflow too
          hN->SetBinContent(0, 0.0); hN->SetBinError(0, 0.0);
          hN->SetBinContent(hN->GetNbinsX()+1, 0.0); hN->SetBinError(hN->GetNbinsX()+1, 0.0);

          named_histograms[h_jer_neg] = hN;
          have_neg = true;
        } else {
          std::cerr << "Missing histogram: " << h_jer_neg << " (and not R2) — skipping.\n";
          continue;
        }
      }

      // Now all inputs are available
      xj_functions::plotxj_and_tworatios(
        canvases[canvas_jer_key],
        named_histograms[h_truth],   // h1    (truth)
        named_histograms[h_unfold],  // horig (central)
        named_histograms[h_jer_pos], // hdiv  (+JER)
        named_histograms[h_jer_neg], // hextra(-JER or synthesized)
        true, "x_{J}", "Unfold / (Unfold #pm JER)",
        0, 1.0, kGreen + 3, 20, 1, 20, kMagenta - 2, 20, 2, 20
      );
      ...
      TLegend* leg_main = nullptr;
      xj_functions::draw_xj_legend(
        leg_main, legend_key.c_str(), 0.61, 0.75, 0.71, 0.90,
        named_histograms[h_truth],
        named_histograms[h_unfold],
        named_histograms[h_jer_pos],
        named_histograms[h_jer_neg],
      ...
      xj_functions::DrawsPHENIXLegendNew(
        leg_sphenix, leg_sphenix_key.c_str(),
        0.13, 0.52, 0.43, 0.90, "p+p #sqrt{s}=200 GeV",
        R_value_str.c_str(), pt_range_str.c_str(), "Matched Truth Dijets",
        true, true, false, true, false
      );
    ...
    for (const auto& gen : gens) {
        for (const auto& R : Rvals) {
            ...
            xj_functions::draw_xj_legend(leg_main, legend_key.c_str(), 0.61, 0.75, 0.71, 0.90,
                ...
                simtype.c_str(), "Unfold", "Unfold + 6% JES", "Unfold - 6% JES", true);
            ...
            std::string outpath_jes = "xj_plot_play_newJER_pdfs/" + canvas_jes_key + ".pdf";
