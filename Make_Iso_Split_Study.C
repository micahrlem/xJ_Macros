void Make_Iso_Split_Study(string infile1 = "pythia8-Jet30-Run21-multiR.root", const std::string configfile = "binning_original.config",float Rvalue = 0.4){
  #include <sstream>
...
#include<array>
  ...
  TH1D *h_xj_classical_truth_all = new TH1D("h_xj_classical_truth_all",";x_{J};1/N", nbins, ixj_bins);
    ...
    TH1D *h_xj_classical_reco_all = new TH1D("h_xj_classical_reco_all",";x_{J};1/N", nbins, ixj_bins);
    TH1D *h_xj_classical_reco_iso = new TH1D("h_xj_classical_reco_iso",";x_{J};1/N", nbins, ixj_bins);
  ...
  if (infile1.find("30") != std::string::npos){
   ...
   float isolated = 0; float split = 0;
   ...
   //int Nentries = 100000;
	...
	if (ntjets < 2 || nrjets < 2 ||  fabs(zvtx) > 60 || fabs(rzvtx) > 60 ){
	   ...
	   for (int k = 0; k < ntjets; k++){//find subleading jets
		 ...
		 if (tjetpt > tsubleadpt){
		   tsubleadpt = tjetpt; tsubleadeta = tjeteta; tsubleadphi = tsubleadphi;
	    ...
	    /*
	     for (int l=0; l < nrjets;l++){ //loop over reco jets
			...
			cout << "Entry: " << i  << " drlead is: " << drlead << " drsublead is: " << drsublead << " drminlead is " << drminlead << " drminsublead is:" << drminsublead << endl;
	    ...
	    */
	        if(candidate == 1 && truth_good){//fill iso hists
	     isolated ++;float xjtruth = tsubleadpt/tleadpt;

	     h_xj_classical_truth_all->Fill(xjtruth,weight);
	     // === 1D pt histograms ===
	     hTruthPtLead_all->Fill(tleadpt, weight);
	     hTruthPtSubLead_all->Fill(tsubleadpt, weight);
	     hTruthPtInclusive_all->Fill(tleadpt, weight);
	     hTruthPtInclusive_all->Fill(tsubleadpt, weight);

	     // === 1D eta histograms ===
	     hTruthEtaLead_all->Fill(tleadeta, weight);
	     hTruthEtaSubLead_all->Fill(tsubleadeta, weight);
	     hTruthEtaInclusive_all->Fill(tleadeta, weight);
	     hTruthEtaInclusive_all->Fill(tsubleadeta, weight);

	     // === 1D phi histograms ===
	     hTruthPhiLead_all->Fill(tleadphi, weight);
	     hTruthPhiSubLead_all->Fill(tsubleadphi, weight);
	     hTruthPhiInclusive_all->Fill(tleadphi, weight);
	     hTruthPhiInclusive_all->Fill(tsubleadphi, weight);

	     // === 1D delta histograms ===
	     hTruthDeltaPhi_all->Fill(tdeltaphi, weight);
	     hTruthDeltaEta_all->Fill(tdeltaeta, weight);

	     // === 2D pt vs phi ===
	     hTruthPt_PhiLead_all->Fill(tleadphi, tleadpt, weight);
	     hTruthPt_PhiSubLead_all->Fill(tsubleadphi, tsubleadpt, weight);
	     hTruthPt_PhiInclusive_all->Fill(tleadphi, tleadpt, weight);
	     hTruthPt_PhiInclusive_all->Fill(tsubleadphi, tsubleadpt, weight);

	     // === 2D pt vs eta ===
	     hTruthPt_EtaLead_all->Fill(tleadeta, tleadpt, weight);
	     hTruthPt_EtaSubLead_all->Fill(tsubleadeta, tsubleadpt, weight);
	     hTruthPt_EtaInclusive_all->Fill(tleadeta, tleadpt, weight);
	     hTruthPt_EtaInclusive_all->Fill(tsubleadeta, tsubleadpt, weight);

	     // === pt1 vs pt2 ===
	     h_pt1pt2_truth_all->Fill(tleadpt, tsubleadpt, weight);
	     h_pt1pt2_truth_all->Fill(tsubleadpt, tleadpt, weight);

	     // === 1D pt histograms ===
	     hTruthPtLead_iso->Fill(tleadpt, weight);
	     hTruthPtSubLead_iso->Fill(tsubleadpt, weight);
	     hTruthPtInclusive_iso->Fill(tleadpt, weight);
	     hTruthPtInclusive_iso->Fill(tsubleadpt, weight);

	     // === 1D eta histograms ===
	     hTruthEtaLead_iso->Fill(tleadeta, weight);
	     hTruthEtaSubLead_iso->Fill(tsubleadeta, weight);
	     hTruthEtaInclusive_iso->Fill(tleadeta, weight);
	     hTruthEtaInclusive_iso->Fill(tsubleadeta, weight);

	     // === 1D phi histograms ===
	     hTruthPhiLead_iso->Fill(tleadphi, weight);
	     hTruthPhiSubLead_iso->Fill(tsubleadphi, weight);
	     hTruthPhiInclusive_iso->Fill(tleadphi, weight);
	     hTruthPhiInclusive_iso->Fill(tsubleadphi, weight);

	     // === 1D delta histograms ===
	     hTruthDeltaPhi_iso->Fill(tdeltaphi, weight);
	     hTruthDeltaEta_iso->Fill(tdeltaeta, weight);

	     // === 2D pt vs phi ===
	     hTruthPt_PhiLead_iso->Fill(tleadphi, tleadpt, weight);
	     hTruthPt_PhiSubLead_iso->Fill(tsubleadphi, tsubleadpt, weight);
	     hTruthPt_PhiInclusive_iso->Fill(tleadphi, tleadpt, weight);
	     hTruthPt_PhiInclusive_iso->Fill(tsubleadphi, tsubleadpt, weight);

	     // === 2D pt vs eta ===
	     hTruthPt_EtaLead_iso->Fill(tleadeta, tleadpt, weight);
	     hTruthPt_EtaSubLead_iso->Fill(tsubleadeta, tsubleadpt, weight);
	     hTruthPt_EtaInclusive_iso->Fill(tleadeta, tleadpt, weight);
	     hTruthPt_EtaInclusive_iso->Fill(tsubleadeta, tsubleadpt, weight);

	     // === pt1 vs pt2 ===
	     h_pt1pt2_truth_iso->Fill(tleadpt, tsubleadpt, weight);
	     h_pt1pt2_truth_iso->Fill(tsubleadpt, tleadpt, weight);
	   ...
	   else if(candidate >= 2 && truth_good){//fill all hists
	     
	     split++;float xjtruth = tsubleadpt/tleadpt;
    ...
    cout << "number of isolated jets: " << isolated << " number of split jets: " << split << endl;
    cout << "proportion of isolated jets: " << isolated*100/(isolated + split) << " % proportion of split jets: " << split*100/(isolated + split) << endl;
    ...
    //Save hists to file for later
     ...
     outfilename.Prepend("Hists_Iso_Split_matchtest_R" + Rvaluestring +"_");
