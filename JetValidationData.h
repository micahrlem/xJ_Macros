...
//  -*- C++ -*-.
#ifndef JETVALIDATIONDATA_H
#define JETVALIDATIONDATA_H
...
class JetValidation : public SubsysReco
{
 public:
  ...
  JetValidation(const std::string &recojetname = "AntiKt_unsubtracted_r04",
 ...
 private:
  ...
  std::vector<float> m_e2;
  std::vector<float> m_pt2;
  std::vector<int> m_cent;
  //! clusters
  /*
  ...
  */
  ...
  std::vector<float> m_truthPt;
   std::vector<float> m_truthEta2;
  std::vector<float> m_truthPhi2;
  std::vector<float> m_truthE2;
  std::vector<float> m_truthPt2;
