...
void make_xJ_Response_smear_flatten(const std::string configfile = "binning_original.config", float RValue = 0.4, string infile1 = "Herwig-Jet10-Run21-multiR.root"){
...
tree->SetBranchAddress("truth_vertex_z",&t_z_vtx);tree->SetBranchAddress("mbd_vertex_z",&r_z_vtx);
 ...
 // identify file to determine setting
    ...
    if(infile1.find("Herwig") != std::string::npos){
      weight = 1.57028*pow(10,-9);  maxpt = 30;eventscale = weight/(1.473*pow(10,-12));
...
//end Miss procedure
    ...
    //define file for writing
     ...
     TString responsepath = "_Smear_Flatten_";
      if (JES_sys > 0)
	    {
